...
\pbox[1.7\width]{
$P_i =$ to be $i^{th}$ execution of the protocol $P$ by either a legitimate user or an adversary. 

$\textbf{s<i>} = $ the $i^{th}$ bit of a bitstring s. 

$s<i,j> = $ the range of bits $s<i>, s<i-1> \dots i<j>$ of a bitstring s. 

$h = $ a hash function.
$\phi\[j\] = $ the $j^{th}$ sub-puzzle in $\phi$. 
}
